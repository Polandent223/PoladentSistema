...
function subscribe(){if(!db())return setTimeout(subscribe,400);db().ref('catalogo_roles_v45').on('value',s=>{catalog=s.val()||{};render()});db().ref('roles_v44').on('value',s=>{asignaciones=s.val()||{};render()});db().ref('configuracion_gps_v51/sedes').on('value',s=>{sedes=s.val()||{};render()});db().ref('empleados').on('value',s=>{empleados=s.val()||{};render()})}
