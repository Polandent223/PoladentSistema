...
(function(){
  ...
  function subscribe(){if(!dbReady())return setTimeout(subscribe,300);window.PoladentData.subscribe('configuracion_gps_v51',s=>{const v=s.val()||{};config={...DEFAULT,...v,sedes:v.sedes||{}};render();});window.PoladentData.subscribe('empleados',s=>{empleados=s.val()||{};renderEmployees();});}
...
(function(){
  ...
  function subscribe(){
    window.PoladentData.subscribe('empleados',s=>{empleados=s.val()||{};setTimeout(patchSave,50);});
    window.PoladentData.subscribe('configuracion_gps_v51/sedes',s=>{sedes=s.val()||{};});
    window.PoladentData.subscribe('alertas_gps',s=>renderAlerts(s.val()||{}));
  }
  function boot(){if(!dbReady())return setTimeout(boot,250);mount();patchSave();subscribe();const refresh=(window.PoladentPerformance?.debounce(()=>{mount();patchSave();},140))||(()=>{mount();patchSave();});new MutationObserver(refresh).observe(document.body,{childList:true,subtree:true});}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
})();
