...
(function(){
  ...
  function augmentListaEmpleados(){
    ...
    cards.forEach(card => {
      if(card.closest && card.closest('.emp-pro-card')) return;
      if(card.classList && card.classList.contains('emp-pro-card')) return;
  ...
  ready(() => {
    ...
    waitFirebase(() => {
      ...
      startObserver();
      bindEmpleadoPin();
      augmentListaEmpleados();
      // Fase 6: estabilización limitada; el MutationObserver y PoladentData mantienen la UI actualizada.
      [250, 800, 1600].forEach(ms => setTimeout(augmentListaEmpleados, ms));
    });
  });
})();
