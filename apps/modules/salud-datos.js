...
(() => {
  'use strict';
  const VERSION='2026.08.08-fase17';
  ...
  function openRoute(route){
    const btn=document.querySelector(`#plu-nav button[data-route="${route}"]`);
    if(btn){btn.click();return true;}
    document.dispatchEvent(new CustomEvent('poladent:route',{detail:{route}}));
    return false;
  }
  function actionFor(x){
    const emp=x.employeeId||'',date=x.date||'';
    if(['HORARIO_GENERAL','HORARIO_INCOMPLETO','HORARIO_ORDEN','SIN_DIAS_LABORABLES'].includes(x.code))return {label:'Abrir horario',kind:'horario',emp,date};
    if(['SECUENCIA_INVALIDA','TIPO_INCONSISTENTE','HORAS_DESORDENADAS','JORNADA_INCOMPLETA_ANTIGUA'].includes(x.code))return {label:'Revisar marcación',kind:'marcacion',emp,date};
    if(['EMPLEADO_SIN_NOMBRE','PIN_FALTANTE','PIN_DUPLICADO','NOMBRE_DUPLICADO'].includes(x.code))return {label:'Ir a empleados',kind:'empleados',emp,date};
    if(x.code==='MARCACIONES_HUERFANAS')return {label:'Abrir auditoría',kind:'auditoria',emp,date};
    return null;
  }
  function actionHtml(x,i){const a=actionFor(x);return a?`<button type="button" class="sd-action" data-issue="${i}">${esc(a.label)}</button>`:''}
  function runAction(issue){
    const a=actionFor(issue);if(!a)return;
    if(a.kind==='horario'){
      if(window.PoladentHorario?.open)return window.PoladentHorario.open(a.emp);
      openRoute('empleados-horarios');
      return;
    }
    if(a.kind==='marcacion'){
      if(window.PoladentCorreccionMarcaciones?.open)return window.PoladentCorreccionMarcaciones.open(a.emp,a.date);
      openRoute('asistencia-correcciones');
      return;
    }
    if(a.kind==='empleados'){
      openRoute('empleados-lista');
      setTimeout(()=>{
        const target=document.querySelector(`[data-employee-id="${CSS.escape(a.emp)}"]`)||document.querySelector(`[data-id="${CSS.escape(a.emp)}"]`);
        target?.scrollIntoView?.({behavior:'smooth',block:'center'});
      },180);
      return;
    }
    if(a.kind==='auditoria'){openRoute('auditoria');return;}
  }
  function renderReport(report){
    ...
    $('#sdIssues').innerHTML=issues.length?issues.map((x,i)=>`<article class="sd-issue"><div class="sd-issue-top"><div>${badge(x.severity)} <b>${esc(x.title)}</b></div>${actionHtml(x,i)}</div><p>${esc(x.detail)}</p><small>${esc(x.code)}${x.date?' · '+esc(x.date):''}${x.employeeId?' · ID '+esc(x.employeeId):''}</small></article>`).join(''):'<div class="sd-clean">✅ No se detectaron inconsistencias en las comprobaciones automáticas.</div>';
    $('#sdIssues').onclick=e=>{const b=e.target.closest('.sd-action');if(!b)return;const item=issues[Number(b.dataset.issue)];if(item)runAction(item);};
  }
  ...
  function mount(){
    ...
    section.innerHTML=`<div class="sd-head"><div><h3>🧹 Salud de datos</h3><p id="sdStamp">Revisión preventiva · no modifica ni borra información</p></div><button type="button" id="sdRun">Revisar ahora</button></div><div id="sdSummary" class="sd-summary"><div><small>Estado</small><b>Sin revisar</b></div></div><details open><summary>Hallazgos que requieren revisión</summary><div id="sdIssues" class="sd-issues"><div class="sd-clean">Pulsa “Revisar ahora”.</div></div></details><p class="sd-note">Esta herramienta es de solo lectura. Los botones solo te llevan al módulo correcto; nunca corrigen, fusionan ni eliminan registros automáticamente.</p>`;
