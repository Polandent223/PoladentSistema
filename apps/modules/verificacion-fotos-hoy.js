...
function subscribe(){
 const database=db();if(!database)return status('Firebase no está disponible.',true);status('Cargando fotografías de hoy…');
 if(state.listener&&typeof state.listener==='function'){try{state.listener()}catch(_e){}state.listener=null;}
 if(window.PoladentData?.subscribe){state.listener=window.PoladentData.subscribe('marcaciones',s=>render(s.val()||{}));}
 else{const ref=database.ref('marcaciones'),handler=s=>render(s.val()||{});ref.on('value',handler,e=>status('No se pudieron cargar las fotografías: '+(e.message||e),true));state.listener=()=>ref.off('value',handler);}
}
