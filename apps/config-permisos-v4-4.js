...
(function(){
  ...
  let empleados={}, marcaciones={}, justificaciones={}, roles={}, config={...DEFAULT};
  ...
  function buildUI(){
    ...
    const wrap=document.createElement('div'); wrap.id='pd44-main'; wrap.innerHTML=`
      ...
      <section id="pd44-roles" class="pd44-panel pd44-tab pd44-hidden">
        ...
        <div class="pd44-actions"><button type="button" id="pd44SaveRole">Guardar rol</button></div><div id="pd44RolesList" class="pd44-list" style="margin-top:12px"></div>
      </section>
  ...
  function bindUI(){
    ...
    $('#pd44SaveRole').onclick=saveRole; $('#pd44EmpSelect').onchange=renderFicha; $('#pd44FichaFecha').onchange=renderFicha; $('#pd44SaveJust').onclick=saveJust; $('#pd44JustEmp').onchange=renderJustList; $('#pd44JustFecha').onchange=renderJustList; $('#pd44Backup').onclick=()=>backup(false); $('#pd44BackupLite').onclick=()=>backup(true);
  }
  ...
  function saveRole(){const email=$('#pd44RoleEmail').value.trim().toLowerCase(); const rol=$('#pd44RoleSelect').value; if(!email)return show('#pd44Status','⚠️ Escribe el correo.'); roles[email]={rol,actualizado:Date.now()}; if(dbReady()) db.ref('roles_v44/'+email.replace(/[.#$\[\]]/g,'_')).set(roles[email]); renderRoles(); show('#pd44Status','✅ Rol guardado.');}
  function renderRoles(){const box=$('#pd44RolesList'); if(!box)return; const items=Object.entries(roles); box.innerHTML=items.length?items.map(([email,r])=>`<div class="pd44-item"><div><b>${esc(email)}</b><br><small>${esc(r.rol)}</small></div><span class="pd44-pill">${esc(r.rol)}</span></div>`).join(''):'<div class="pd44-item"><small>No hay roles configurados todavía.</small></div>';}
  ...
  function renderAll(){if(!$('#pd44-main'))return; renderMini(); renderSelects(); renderRoles(); renderFicha(); renderJustList();}
  ...
  function subscribe(){if(!dbReady())return setTimeout(subscribe,300); try{db.ref('configuracion_v44').on('value',s=>{if(s.exists()){config={...DEFAULT,...config,...s.val()}; localSave(); fillForms(); renderAll();}}); db.ref('empleados').on('value',s=>{empleados=s.val()||{}; renderAll();}); db.ref('marcaciones').on('value',s=>{marcaciones=s.val()||{}; renderAll();}); db.ref('justificaciones_v44').on('value',s=>{justificaciones=s.val()||{}; renderAll();}); db.ref('roles_v44').on('value',s=>{roles=s.val()||{}; renderAll();});}catch(e){setTimeout(subscribe,800);}}
