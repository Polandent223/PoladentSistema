...
(function(){
  ...
  let empleadoActual=null,marcando=false,configReady=false,etapaPendiente=null,gpsConfig={activo:false,precisionMaxima:120,sedes:{}},segConfig={activo:true,fotoEntrada:true,fotoAlmuerzoSalida:false,fotoAlmuerzoRegreso:false,fotoSalida:true,alertarDispositivoCompartido:true,ventanaDispositivoMinutos:10,guardarEvidencia:true,maxIntentosPin:5,bloqueoPinMinutos:5};
  ...
  async function buscarEmpleado(pin){const s=await db().ref('empleados').orderByChild('pin').equalTo(pin).once('value');if(!s.exists())return null;let out=null;s.forEach(x=>{const e=x.val()||{};if(!out&&e.archivado!==true&&e.activo!==false)out={id:x.key,...e};});return out;}
