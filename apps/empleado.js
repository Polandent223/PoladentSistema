/* POLADENT v5.3 - Empleado rápido, asignación obligatoria y alertas GPS */
(function(){
  'use strict';
  ...
  function validarSede(gps){
    ...
    const permitidas=empleadoActual.sedesPermitidas||{};
    if(!Object.keys(permitidas).length)return {ok:false,codigo:'SIN_SEDE',error:'Todavía no estás agregado a ninguna tienda oficial. Comunícate con el administrador.'};
    sedes=sedes.filter(([id])=>permitidas[id]);
    if(!sedes.length)return {ok:false,codigo:'SEDE_INACTIVA',error:'La tienda que tienes asignada no está activa. Comunícate con el administrador.'};
    ...
    if(best.distancia>radio)return {ok:false,codigo:'FUERA_AREA',sedeCercana:best.s.nombre,distancia:best.distancia,radio,error:`Estás fuera del área autorizada. Local más cercano: ${best.s.nombre} a ${Math.round(best.distancia)} m; límite ${radio} m.`};
    return {ok:true,sede:{id:best.id,nombre:best.s.nombre,direccion:best.s.direccion||'',radio},distancia:best.distancia};
  }
  async function registrarIntentoGps(valid,gps,tipo){
    try{
      const ref=db().ref('alertas_gps').push();
      await ref.set({
        empleadoId:empleadoActual?.id||null,
        empleado:empleadoActual?.nombre||'Empleado desconocido',
        tipo:tipo||null,
        codigo:valid?.codigo||'GPS_BLOQUEADO',
        motivo:valid?.error||'Intento GPS bloqueado',
        sedeCercana:valid?.sedeCercana||null,
        distancia:Number.isFinite(Number(valid?.distancia))?Math.round(Number(valid.distancia)):null,
        radioPermitido:Number.isFinite(Number(valid?.radio))?Number(valid.radio):null,
        lat:gps?.lat??null,lon:gps?.lon??null,
        precisionGps:gps?.accuracy?Math.round(gps.accuracy):null,
        timestamp:Date.now(),fecha:todayKey(new Date()),
  ...
  async function mark(tipo){if(marcando)return;if(!empleadoActual)return msg('Primero ingresa un PIN válido.','error');setBusy(true);msg(gpsConfig.activo?'Verificando ubicación GPS…':'Guardando marcación, espere…');try{
    ...
    const gps=await getGps(),valid=validarSede(gps);if(!valid.ok){await registrarIntentoGps(valid,gps,tipo);throw new Error(valid.error);}const hora=now.toLocaleTimeString('es-VE'),timestamp=now.getTime();const data={nombre:empleadoActual.nombre||'Empleado',tipo,fecha,hora,timestamp,lat:gps.lat,lon:gps.lon,precisionGps:Math.round(gps.accuracy),gpsVerificado:!!gpsConfig.activo};if(valid.sede){Object.assign(data,{sedeId:valid.sede.id,sedeNombre:valid.sede.nombre,sedeDireccion:valid.sede.direccion,distanciaSede:Math.round(valid.distancia),radioPermitido:valid.sede.radio});}
