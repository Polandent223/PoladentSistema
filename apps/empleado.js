...
(function(){
  ...
  let empleadoActual=null,marcando=false,gpsConfig={activo:false,precisionMaxima:120,sedes:{}},segConfig={activo:true,fotoEntrada:true,fotoAlmuerzoSalida:true,fotoAlmuerzoRegreso:true,fotoSalida:true,alertarDispositivoCompartido:true,ventanaDispositivoMinutos:10,guardarEvidencia:true,maxIntentosPin:5,bloqueoPinMinutos:5};
  ...
  function requiresPhoto(tipo){if(segConfig.activo===false)return false;if(segConfig.fotosTemporalesDiarias!==false)return true;const per=empleadoActual?.seguridadFotos||{};const key={entrada:'fotoEntrada',almuerzo_salida:'fotoAlmuerzoSalida',almuerzo_regreso:'fotoAlmuerzoRegreso',salida:'fotoSalida'}[tipo];return per[key]!==undefined?!!per[key]:!!segConfig[key];}
