...
(function(){
  ...
  async function ensureEmployeeSession(){
    ...
    const existing=await new Promise(resolve=>{
      let finished=false;
      const timer=setTimeout(()=>{if(!finished){finished=true;resolve(auth.currentUser||null);}},2500);
      const off=auth.onAuthStateChanged(user=>{if(!finished){finished=true;clearTimeout(timer);off();resolve(user||null);}});
    });
  ...
  let buscandoPin=false,lastInvalid='',invalidAttempts=Number(sessionStorage.getItem('poladent_invalid_pin_attempts')||0),lockUntil=Number(sessionStorage.getItem('poladent_pin_lock_until')||0);
  ...
  async function submitPinSearch(){
    ...
    try{
      ...
      msg(gpsConfig.activo?`${e.nombre||'Empleado'} encontrado. La ubicación se verificará al marcar.`:`${e.nombre||'Empleado'} encontrado. Selecciona la marcación.`,'ok');
    }catch(error){
  ...
  ready(()=>{
    updateClock();ensurePreflightUi();runPreflight();setInterval(updateClock,1000);ensureCameraUi();
    waitFirebase(async()=>{
      try{
        ...
        msg('Escribe tu PIN completo y pulsa Buscar.','ok');
      }catch(error){
