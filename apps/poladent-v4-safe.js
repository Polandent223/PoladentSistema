...
(function(){
  ...
  ready(()=>{
    ...
    injectStyles(); bind();
    waitFirebase(()=>{
      firebase.database().ref('empleados').on('value', snap=>{ empleados=snap.val()||{}; renderSelect(); });
    });
  });
})();
...
(function(){
  ...
  function start(){
    firebase.database().ref('empleados').on('value',snap=>{ cache=snap.val()||{}; renderSelect(); });
