...
(function(){
  ...
  function organizeAdmin(){
    ...
    if(!panel || panel.dataset.pd42Ready === '1') return;
    ...
    original.forEach(node=>{
      ...
      const id = node.id || '';
      if(node.classList.contains('adminTopBar') || node.classList.contains('dashboardHero') || node.classList.contains('kpiGrid') || node.classList.contains('chartPremium')){
