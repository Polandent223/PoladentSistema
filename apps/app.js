...
// 👷 EMPLEADO – PIN + BOTONES DINÁMICOS + CONTROL DE ORDEN
...
function mark(tipo){
  ...
  if (lastIndex === -1 && tipo !== 'entrada') {
    document.getElementById("empMsg").innerHTML =
      "⚠️ Debes iniciar con Entrada";
    return;
  }

  if (lastIndex !== -1 && etapas.indexOf(tipo) !== lastIndex + 1) {
    document.getElementById("empMsg").innerHTML =
      "⚠️ Debes seguir el orden de marcación";
    return;
  }
...
// Actualiza gráfico al cambiar rango
...
// 🔽 Función para minimizar/expandir listas
