...
function cargarEmpleados() {
  ...
  window.PoladentData.subscribe("empleados", (snap) => {
    ...
    const prevFilter = document.getElementById("empDirectoryFilter")?.value || "activos";
    ...
    const render = () => {
      ...
      const f = document.getElementById("empDirectoryFilter")?.value || "activos";
      ...
      if (count) count.textContent = `${filtered.length} funcionario${filtered.length===1?"":"s"}`;
      if (!cards) return;
      ...
      .forEach((fecha) => {
        ...
        Object.keys(tipos)
          ...
          .forEach((tipo) => {
            ...
            notif.innerHTML += notificationCard(data, fecha);
          });
      });
  });
