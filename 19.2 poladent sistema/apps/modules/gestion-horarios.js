...
function subscribe(){
 ...
 if(window.PoladentData?.subscribe){
   window.PoladentData.subscribe('empleados',s=>{empleados=s.val()||{};mount();renderEmployees();});
 }else{
   const native=firebase.database().ref('empleados'); native.on('value',s=>{empleados=s.val()||{};mount();renderEmployees();},e=>notice('No se pudieron cargar empleados: '+(e.message||e),false));
 }
}
