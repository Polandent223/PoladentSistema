...
async function loadAccess(user){
 ...
 try{
   const userRec=(await d.ref('usuarios_admin/'+key(email)).once('value')).val();
   ...
   const a=(await d.ref('roles_v44/'+key(email)).once('value')).val();
   ...
   const custom=(await d.ref('permisos_roles_v44/'+role).once('value')).val();
   if(custom&&Array.isArray(custom.permisos))perms=custom.permisos;
   const cat=(await d.ref('catalogo_roles_v45/'+role).once('value')).val();
...
function init(){
 ...
 const mo=new MutationObserver(()=>{mountBadge();if(ready)applyNav()});mo.observe(document.body,{childList:true,subtree:true});
}
